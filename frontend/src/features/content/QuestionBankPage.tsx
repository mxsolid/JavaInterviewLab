...
import { difficultyLabel, frequencyLabel } from '../study/labels';
...
export function QuestionBankPage() {
  ...
  const [filter, setFilter] = useState<Filter>(() => ({ page: 1, pageSize: 10, status: 'ENABLED', keyword: searchParams.get('keyword') || undefined, topicId: Number(searchParams.get('topicId')) || undefined })); const [editing, setEditing] = useState<QuestionDetail | undefined>(); const [editorOpen, setEditorOpen] = useState(false); const navigate = useNavigate();
  useEffect(() => {
    const keyword = searchParams.get('keyword') || undefined;
    const topicId = Number(searchParams.get('topicId')) || undefined;
    setFilter((current) => current.keyword === keyword && current.topicId === topicId ? current : { ...current, keyword, topicId, page: 1 });
  }, [searchParams]);
  ...
  const columns: ColumnsType<QuestionSummary> = [{ title: '题目', dataIndex: 'title', render: (title, row) => <Button type="link" onClick={() => navigate(`/questions/${row.id}`)}>{title}</Button> }, { title: '专题', dataIndex: 'topicName' }, { title: '星级', dataIndex: 'starLevel', render: (value) => <StarRating value={value} /> }, { title: '难度', dataIndex: 'difficulty', render: (value) => <DifficultyTag value={value} /> }, { title: '高频度', dataIndex: 'frequencyLevel', render: (value) => <FrequencyTag value={value} /> }, { title: '操作', render: (_, row) => <Button icon={<EditOutlined />} onClick={() => void openEdit(row.id)}>编辑</Button> }];
